package NCarton::Snapshot::Parser;
use Class::Tiny;
...
my $machine = {
    ...
    dists => [
        {
            re => qr/^  (\S+)$/,
            code => sub { $_[0]->{dist} = NCarton::Dist->new(name => $1) },
            goto => 'distmeta',
        },
        {
            re => qr/^\S/,
...
sub parse {
    ...
    STATE: {
            ...
            NCarton::Error::SnapshotParseError->throw(error => "Could not parse snapshot file: $line");
        }
    }
}
