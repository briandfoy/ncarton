package NCarton::Environment;
use strict;
use NCarton::CPANfile;
...
sub _build_snapshot {
    my $self = shift;
    NCarton::Snapshot->new(path => $self->cpanfile . ".snapshot");
}
...
sub _build_tree {
    my $self = shift;
    NCarton::Tree->new(cpanfile => $self->cpanfile, snapshot => $self->snapshot);
}
...
sub build_with {
    ...
    if ($cpanfile->is_file) {
        return $class->new(cpanfile => NCarton::CPANfile->new(path => $cpanfile));
    } else {
        NCarton::Error::CPANfileNotFound->throw(error => "Can't locate cpanfile: $cpanfile");
    }
}
...
sub build {
    ...
    if ($cpanfile && $cpanfile->is_file) {
        $self->cpanfile( NCarton::CPANfile->new(path => $cpanfile) );
    } else {
        NCarton::Error::CPANfileNotFound->throw(error => "Can't locate cpanfile: (@{[ $cpanfile_path || 'cpanfile' ]})");
    }
