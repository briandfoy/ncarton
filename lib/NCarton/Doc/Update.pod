...
NCarton::Doc::Update - Update the dependencies

=head1 SYNOPSIS

  cartone update [module]

=head1 DESCRIPTION

Update the dependencies version for your application.

NCarton is designed to update your dependency in a conservative way,
meaning that it doesn't update modules that aren't explicitly required
to.

C<cartone update> is a command to explicitly update one or all of
...
and then run C<cartone install> to get DBI 1.610 (the latest at that
time) and Plack 1.0011 (as specified in the requirement).

A few weeks later, DBI and Plack have been updated a couple of
times. Running C<cartone install> I<won't> update the versions, because
the installed versions satisfy the requirements in C<cpanfile>.

Running C<cartone update> will update DBI to the latest version, say
