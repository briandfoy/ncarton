package NCarton::Snapshot;
use strict;
use Config;
use NCarton::Dist;
use NCarton::Dist::Core;
use NCarton::Error;
use NCarton::Package;
use NCarton::Index;
use NCarton::Util;
use NCarton::Snapshot::Emitter;
use NCarton::Snapshot::Parser;
use CPAN::Meta;
...
sub load {
    ...
    if ($self->path->is_file) {
        my $parser = NCarton::Snapshot::Parser->new;
        $parser->parse($self->path->slurp_utf8, $self);
        $self->loaded(1);

        return 1;
    } else {
        NCarton::Error::SnapshotNotFound->throw(
            error => "Can't find cpanfile.snapshot: Run `cartone install` to build the snapshot file.",
            path => $self->path,
        );
    }
}
...
sub save {
    my $self = shift;
    $self->path->spew_utf8( NCarton::Snapshot::Emitter->new->emit($self) );
}
...
sub find_in_core {
    ...
    if (exists $Module::CoreList::version{$]}{$module}) {
        ...
        return NCarton::Dist::Core->new(name => $module, module_version => $version);
    }
...
sub packages {
    ...
    for my $dist ($self->distributions) {
        while (my($package, $provides) = each %{$dist->provides}) {
            ...
            push @packages, NCarton::Package->new($package, $provides->{version}, $dist->pathname);
        }
    }
...
sub find_installs {
    ...
    for my $file (@installs) {
        my $module = NCarton::Util::load_json($file->[0]);
        ...
        if ($accepts->($module)) {
            $installs{$module->{name}} = NCarton::Dist->new(
                name => $module->{dist},
