...
use NCarton::Builder;
use NCarton::Mirror;
use NCarton::Snapshot;
use NCarton::Util;
use NCarton::Environment;
use NCarton::Error;
...
sub _build_mirror {
    my $self = shift;
    NCarton::Mirror->new($ENV{PERL_CARTON_MIRROR} || $NCarton::Mirror::DefaultMirror);
}
...
sub run {
    ...
    my $code = try {
        my $call = $self->can("cmd_$cmd")
            or NCarton::Error::CommandNotFound->throw(error => "Could not find command '$cmd'");
        $self->$call(@commands);
        return 0;
    } catch {
        die $_ unless blessed $_ && $_->can('rethrow');

        if ($_->isa('NCarton::Error::CommandExit')) {
            return $_->code || 255;
        } elsif ($_->isa('NCarton::Error::CommandNotFound')) {
            warn $_->error, "\n\n";
            $self->cmd_usage;
            return 255;
        } elsif ($_->isa('NCarton::Error')) {
            warn $_->error, "\n";
            return 255;
        }
    };
...
sub cmd_help {
    my $self = shift;
    my $module = $_[0] ? ("NCarton::Doc::" . ucfirst $_[0]) : "NCarton.pm";
...
sub cmd_install {
    ...
    if ($cached) {
        $builder->mirror(NCarton::Mirror->new($env->vendor_cache));
    }
...
sub cmd_tree {
    ...
    my $dumper = sub {
        ...
        return if $level == 0;
        return NCarton::Tree::STOP if $dependency->dist->is_core;
        return NCarton::Tree::STOP if $seen{$dependency->distname}++;
...
sub cmd_check {
    ...
    my $env = NCarton::Environment->build($cpanfile_path);
    $env->snapshot->load;
    $env->cpanfile->load;
